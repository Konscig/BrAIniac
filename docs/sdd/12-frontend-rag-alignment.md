...
## Dataset Artifact Index Update 2026-04-25

- Frontend does not need a new indexing UI for the current MVP. When the user selects a dataset and starts execution, backend prepares or reuses `input_json.dataset_index`.
- The run drawer may show backend warnings such as dataset artifact index built/reused. These diagnostics are informational and should not block canvas editing.
- For document questions, `AgentCall` is guided to prefer `QueryBuilder -> HybridRetriever -> ContextAssembler -> LLMAnswer` when `dataset_index` is already present, instead of rebuilding documents/chunks/vectors every time.
- Debug output should make it visible whether retrieval used artifact vectors (`retrieval_source: "artifact-vectors"`) or found no candidates (`retrieval_source: "no-results"`).
