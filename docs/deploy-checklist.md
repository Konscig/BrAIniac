...
<!-- Redis Runtime Production Notes -->

- [ ] Redis is reachable only inside the private Docker/VPC network in production.
- [ ] `REDIS_REQUIRED=true` is set for production.
- [ ] `REDIS_KEY_PREFIX` is unique per environment.
- [ ] `/runtime/health` is monitored separately from `/health`.
- [ ] Redis outage playbook is verified: refresh sessions, rate limits, execution coordination, queues, and provider cooldowns fail closed; cache and progress degrade to direct compute/polling.
