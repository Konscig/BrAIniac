...
type RunTask struct {
	...
	RunID      uuid.UUID `gorm:"foreignKey:RunID;constraint:OnDelete:CASCADE"`
	NodeID     uuid.UUID `gorm:"foreignKey:NodeID;constraint:OnDelete:CASCADE"`
	Worker     string    `gorm:"type:text;not null"`
	Status     string    `gorm:"type:text;not null"`
	Attempt    int       `gorm:"not null"`
	Metric     uuid.UUID `gorm:"foreignKey:RunID;constraint:OnDelete:CASCADE"`
