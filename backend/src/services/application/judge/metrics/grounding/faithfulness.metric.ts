...
export class FaithfulnessMetric extends MetricBase {
  readonly code = 'f_faith';
  ...
  async compute(ctx: MetricContext): Promise<MetricResult> {
    ...
    for (const it of ctx.items) {
      if (typeof it.agent_output.text !== 'string' || !Array.isArray(it.agent_output.context)) continue;
      const res = await computeMetric(this.code, {
        agent_output: {
          text: it.agent_output.text,
          context: it.agent_output.context,
          claims: it.gold?.claims,
        },
      });
